...
async function fetchSmartMoney(markPrice: number) {
  ...
  allSettledResults.forEach((result, idx) => {
    ...
    const solPos = state.assetPositions.find((p: any) => p.position?.coin === 'SOL');
    if (!solPos) return;

    const szi = parseFloat(solPos.position.szi);
    const unrealizedPnl = parseFloat(solPos.position.unrealizedPnl);
    const leverage = parseFloat(solPos.position.leverage?.value || 1);
  ...
  if (total === 0) return { longPct: 50, shortPct: 50, ratio: 1, sentiment: 'NEUTRAL', netUsd: 0, longUsd: 0, shortUsd: 0, longCount: 0, shortCount: 0, wallets: [] };
...
async function fetchOHLCV(timeframe: string, days: number) {
  ...
  return hlPost({
    type: 'candleSnapshot',
    req: { coin: 'SOL', interval, startTime: start, endTime: end },
  });
}
...
async function fetchMetaAndCtxs() {
  ...
  const idx = meta.universe.findIndex((a: any) => a.name === 'SOL');
  if (idx === -1) throw new Error('SOL not found in universe');
  
  return { meta: meta.universe[idx], ctx: ctxs[idx], allMeta: meta };
}
...
async function fetchFunding() {
  ...
  return hlPost({
    type: 'fundingHistory',
    req: { coin: 'SOL', startTime: start, endTime: end },
  });
}
