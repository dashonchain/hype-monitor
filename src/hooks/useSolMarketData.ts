...
const PRICE_REFRESH = 5_000;
const SMART_MONEY_REFRESH = 30_000;
const REST_REFRESH = 60_000;

export interface DerivativesData {
  longShortRatio: { ratio: number; ratioDelta: number; longPct: number; shortPct: number; longTotalUsd: number; shortTotalUsd: number; buyVolume24h: number; sellVolume24h: number; delta24h: number; interpretation: string; source: string };
...
export function useSolMarketData(initialTf: Timeframe = '4h') {
  ...
  const fetchData = useCallback(async (timeframe?: Timeframe, silent = false) => {
    ...
    try {
      ...
      const d = await fetch(`/api/sol?timeframe=${timeframe || tf}`).then(r => r.json());
      setData(d);
      setFetchCount(c => c + 1);
      setError('');
    } catch (e: any) {
  ...
  useEffect(() => {
    fetchData();
    fetchDerivatives();
    const priceInterval = setInterval(() => fetchData(undefined, true), PRICE_REFRESH);
    const smartMoneyInterval = setInterval(() => fetchData(undefined, true), SMART_MONEY_REFRESH);
    const restInterval = setInterval(() => fetchData(), REST_REFRESH);
    return () => {
